...
- (NSMutableArray *)columnNameArray //zly added
{
    if (!_columnNameArray) {
        int columnCount = sqlite3_column_count([_statement statement]);
        _columnNameArray = [[NSMutableArray alloc] initWithCapacity:(NSUInteger)columnCount];
        int columnIdx = 0;
        for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
            NSString *columnName = [NSString stringWithUTF8String:sqlite3_column_name([_statement statement], columnIdx)];
            [_columnNameArray addObject:columnName];
        }
    }
...
- (NSArray*)resultArray {//zly added
    int dataCount = sqlite3_data_count([_statement statement]);
    int columnCount = sqlite3_column_count([_statement statement]);
    if (dataCount != columnCount){
        //NSLog(@"in resultArray, dataCount != columnCount");
    }
    
    NSMutableArray *ary = [NSMutableArray arrayWithCapacity:columnCount];
    int columnIdx = 0;
    for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
        id objectValue = [self objectForColumnIndex:columnIdx];
        [ary addObject:objectValue];
    }
    return ary;
}
